...
var ANO_JSONS = ['ano.json', '.ano.json'];
var LIB_CONFS = ['library.json', 'library.properties'];
var POSSIBLE_CONFS = ANO_JSONS.concat(LIB_CONFS);
...
function read(file, options, callback) {
  ...
  fs.stat(file, function (err, stat) {
    ...
    if (stat.isDirectory()) {
      // If strict, we find ano jsons only
      possibles = options.strict ? ANO_JSONS : POSSIBLE_CONFS;
...
//
// https://github.com/ano/ano.json-spec
